...
import { SessionList, Session, SessionStorage } from "../interfaces";
...
export default class SessionStore extends Store {
    ...
    set(sessionId: string, session: SessionStorage, callback?: (error: Error, session?: Session) => void): void {
        try {
            ...
            this.db.get(`INSERT OR REPLACE INTO ${this.table} VALUES (?, ?, ?, ?)`, [sessionId, expired, rawSession, null], (err: Error, rows) => {
                if (err) callback(err);
